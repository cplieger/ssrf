...
| DNS rebinding (validate one IP, connect to another) | resolve-once + `Dialer.Control` re-validates the *actually-connected* IP at dial time | transport tests |
...
- The library protects requests routed *through* it; a consumer that makes a
